                ...
                <div>
                    @php
                        ...
                        $statusLabel = match($proposal->status) {
                            ...
                            'direkomendasikan' => 'Sedang Diverifikasi Pusat',
                            'ditolak' => 'Di tolak',
                        ...
                        @else
                            <div class="grid grid-cols-2 gap-y-5 gap-x-4">
                                ...
                                <div>
                                    ...
                                    <p class="text-gray-900 font-medium">{{ $proposal->program->close_date ? $proposal->program->close_date->translatedFormat('d F Y') : '-' }}</p>
                                </div>
                                <div>
                ...
                <div class="space-y-6">
                    <div class="bg-gray-50 overflow-hidden shadow-sm sm:rounded-2xl border border-gray-200 p-6">
                        ...
                        <div class="relative border-l-2 border-primary-200 ml-3 space-y-8 pb-2">
                            ...
                            @if(!$rejectedByAdmin && !($rejectedAtDisposisi && !$rejectedAtFinal) && !(in_array($proposal->status, ['disetujui', 'dikembalikan', 'direkomendasikan', 'ditolak_pusat']) && !$surveyAssignment))
                            <div class="relative">
                            ...
                            <div class="relative">
                                ...
                                @elseif($proposal->status === 'ditolak_pusat')
                                    ...
                                    <div class="pl-4">
                                        ...
                                        <p class="text-[10px] text-gray-400 font-bold uppercase tracking-wider">{{ $proposal->decided_at ? $proposal->decided_at->translatedFormat('d M Y - H:i') : $proposal->updated_at->translatedFormat('d M Y - H:i') }} WIB</p>
                                        <p class="text-xs text-red-500 mt-1">Alasan: {{ str_replace('DITOLAK PUSAT: ', '', $proposal->pimpinan_notes ?? '-') }}</p>
                                    </div>
