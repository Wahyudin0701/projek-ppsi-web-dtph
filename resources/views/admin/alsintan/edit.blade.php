        ...
        <div class="flex items-center justify-between mb-2">
            <div>
                <div class="flex items-center gap-2 text-sm text-gray-500 mb-1">
                    ...
                    <span class="font-semibold text-gray-700">Edit Alat</span>
                </div>
            ...
            <div class="p-6 text-gray-900">
                <form action="{{ route('admin.alsintan.update', $alsintan) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div>
                            <label for="stock" class="block text-sm font-bold text-gray-700 mb-2">Total Stok Milik Dinas</label>
                            <input type="number" name="stock" id="stock" value="{{ old('stock', $alsintan->stock) }}" min="0" required
                                   class="w-full rounded-xl border-gray-300 focus:border-primary-500 focus:ring-primary-500 shadow-sm">
                            <x-input-error :messages="$errors->get('stock')" class="mt-2" />
                        </div>
                        <div>
                            <label for="borrowed_count" class="block text-sm font-bold text-gray-700 mb-2">Sedang Dipinjam</label>
                            <input type="number" name="borrowed_count" id="borrowed_count" value="{{ old('borrowed_count', $alsintan->borrowed_count) }}" min="0" required
                                   class="w-full rounded-xl border-gray-300 focus:border-primary-500 focus:ring-primary-500 shadow-sm">
                            <x-input-error :messages="$errors->get('borrowed_count')" class="mt-2" />
                        </div>
                        <div>
                            <label for="broken_count" class="block text-sm font-bold text-gray-700 mb-2">Rusak / Perbaikan</label>
                            <input type="number" name="broken_count" id="broken_count" value="{{ old('broken_count', $alsintan->broken_count) }}" min="0" required
                                   class="w-full rounded-xl border-gray-300 focus:border-primary-500 focus:ring-primary-500 shadow-sm">
