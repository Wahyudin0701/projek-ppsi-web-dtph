<x-app-layout>
    <x-slot name="header">Tambah Jenis Program</x-slot>
    <div class="max-w-7xl mx-auto space-y-6">
        {{-- Header --}}
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-2xl font-extrabold text-gray-900">Tambah Jenis Program</h2>
                <p class="text-gray-500 text-sm mt-1">Tambahkan jenis program bantuan yang baru.</p>
            </div>
        ...
        <div class="bg-white overflow-hidden shadow-sm rounded-2xl border border-gray-100">
            <form action="{{ route('admin.program-categories.store') }}" method="POST" class="p-6 md:p-8 space-y-6">
                @csrf
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="3"
                                  class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all"
                                  placeholder="Deskripsi singkat mengenai jenis program ini...">{{ old('description') }}</textarea>
                        @error('description')
                            <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                        @enderror
                    </div>
