<x-app-layout>
    ...
    <x-slot:header>Edit Artikel</x-slot:header>
    ...
    <form action="{{ route('admin.articles.update', $article) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-3xl border border-gray-100 shadow-[0_4px_20px_-10px_rgba(0,0,0,0.05)] p-8">
        @csrf
        @method('PUT')
            ...
            <div class="md:col-span-2 space-y-6">
                ...
                <div>
                    ...
                    <textarea name="summary" id="summary" rows="3" required
                        class="block w-full px-4 py-3 border border-gray-300 rounded-xl text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-600/20 focus:border-blue-600 transition-all">{{ old('summary', $article->summary) }}</textarea>
                </div>
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <select name="category" id="category" required
                        ...
                        <option value="program" {{ old('category', $article->category) == 'program' ? 'selected' : '' }}>Program</option>
                        <option value="artikel" {{ old('category', $article->category) == 'artikel' ? 'selected' : '' }}>Artikel Edukasi</option>
                        <option value="kegiatan" {{ old('category', $article->category) == 'kegiatan' ? 'selected' : '' }}>Kegiatan Lapangan</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="author_name" id="author_name" value="{{ old('author_name', $article->author_name) }}" required
                        class="block w-full px-4 py-3 border border-gray-300 rounded-xl text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-600/20 focus:border-blue-600 transition-all">
                </div>

                <!-- Foto Thumbnail -->
                <div>
                    <label for="image" class="block text-sm font-bold text-gray-800 mb-2">Foto / Thumbnail Baru <span class="text-xs text-gray-400 font-normal">(Opsional)</span></label>
                    
                    @if($article->image_path)
                        <div class="mb-3">
                            <p class="text-xs text-gray-500 mb-1">Foto saat ini:</p>
                            <img src="{{ Storage::url($article->image_path) }}" alt="Thumbnail" class="w-full h-32 object-cover rounded-xl border border-gray-200">
                        </div>
                    @endif
                ...
                <div class="pt-4 border-t border-gray-100">
                    <label class="flex items-center gap-3 cursor-pointer">
                        ...
                        <span class="text-sm font-bold text-gray-900">Publikasikan</span>
                    </label>
                </div>
