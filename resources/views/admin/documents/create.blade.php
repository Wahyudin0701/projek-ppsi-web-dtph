<x-app-layout>
    ...
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl md:text-3xl font-extrabold text-gray-900 tracking-tight">Tambah Dokumen Baru</h1>
            <p class="text-sm text-gray-500 mt-1">Unggah dokumen baru untuk ditampilkan di halaman publik.</p>
        </div>
        <a href="{{ route('admin.documents.index') }}" class="inline-flex items-center justify-center gap-2 px-5 py-2.5 bg-white border border-gray-200 text-gray-700 text-sm font-bold rounded-xl hover:bg-gray-50 transition-colors shadow-sm">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
        <form action="{{ route('admin.documents.store') }}" method="POST" enctype="multipart/form-data" class="p-6 md:p-8">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <input type="text" name="title" id="title" value="{{ old('title') }}" required
                               ...
                               placeholder="Contoh: Permentan Nomor 15 Tahun 2025">
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="3"
                                  class="w-full px-4 py-3 rounded-xl border border-gray-300 focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-colors shadow-sm"
                                  placeholder="Tambahkan keterangan singkat mengenai dokumen ini (opsional)...">{{ old('description') }}</textarea>
                    </div>
                ...
                <div class="space-y-6">
                    <div>
                        <label class="block text-sm font-bold text-gray-700 mb-2">Unggah File <span class="text-red-500">*</span></label>
                            ...
                            <div class="space-y-2 text-center relative z-10">
                                ...
                                <div class="flex text-sm text-gray-600 justify-center">
                                    <label for="file" class="relative cursor-pointer bg-white rounded-md font-bold text-primary-600 hover:text-primary-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-primary-500">
                                        ...
                                        <input id="file" name="file" type="file" class="sr-only" required accept=".pdf,.doc,.docx,.xls,.xlsx,.zip,.rar">
                                    </label>
                                    <p class="pl-1">atau drag and drop</p>
                                </div>
                                <p class="text-xs text-gray-500">PDF, DOCX, XLSX, ZIP hingga 20MB</p>
                    ...
                    <div class="p-5 bg-gray-50 border border-gray-100 rounded-xl">
                        ...
                        <div class="flex items-start">
                            <div class="flex items-center h-5">
                                <input id="is_public" name="is_public" type="checkbox" value="1" {{ old('is_public', true) ? 'checked' : '' }} class="focus:ring-primary-500 h-5 w-5 text-primary-600 border-gray-300 rounded cursor-pointer">
                            </div>
            ...
            <div class="mt-8 pt-6 border-t border-gray-100 flex justify-end gap-4">
                ...
                <button type="submit" class="px-8 py-3 bg-primary-600 text-white font-bold rounded-xl hover:bg-primary-700 transition-colors shadow-md hover:shadow-lg">Simpan Dokumen</button>
            </div>
        </form>
    </div>
