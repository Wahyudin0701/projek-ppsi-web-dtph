        ...
        <tr>
            ...
            <td>
                <table style="width:100%; border:none; margin:0; padding:0; border-collapse:collapse;">
                    ...
                    @endphp
                    <tr><td style="width:25px; vertical-align:top; padding:0;">1.</td><td style="vertical-align:top; padding:0; text-align:justify;">Melaksanakan perjalanan dinas dalam rangka verifikasi lokasi {{ $proposal->alsintan_id ? 'peminjaman alsintan ' . $proposal->alsintan->name : 'bantuan ' . $proposal->program->name }} oleh <strong>{{ $proposal->user->farmerProfile->nama_kelompok ?? $proposal->user->name }}</strong> di Desa {{ ucwords(strtolower($proposal->user->farmerProfile->alamat ?? '-')) }} Kecamatan {{ ucwords(strtolower($proposal->user->farmerProfile->kecamatan ?? '-')) }} selama {{ $lamaHari }} hari pada {{ $tanggalTeks }}</td></tr>
