            ...
            <div class="md:col-span-2 space-y-6">
                ...
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 sm:p-8">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-y-6 gap-x-4">
                        <div>
                            ...
                            <p class="text-gray-900 font-semibold text-sm">{{ $proposal->user->farmerProfile->nama_kelompok ?? '-' }}</p>
                        </div>
                        <div>
                            <p class="text-xs font-bold text-gray-400 uppercase tracking-widest mb-1">Nama Ketua</p>
                            <p class="text-gray-900 font-semibold text-sm">{{ $proposal->user->farmerProfile->ketua ?? $proposal->user->name }}</p>
                        </div>
                        <div>
                ...
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 sm:p-8">
                    <div class="flex items-center justify-between mb-6 border-b border-gray-100 pb-3">
                        ...
                        <a href="{{ route('documents.cpcl', $proposal) }}" target="_blank" class="text-xs text-primary-600 font-bold hover:underline flex items-center gap-1.5">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z"/></svg>
                            Cetak PDF
                        </a>
                    </div>
                    @foreach($proposal->cpclVerifications as $cpcl)
                    <div class="bg-orange-50/40 rounded-xl p-5 border border-orange-100/50 mb-4 last:mb-0">
                        
                        {{-- Verifikasi Administrasi --}}
                        <div class="mb-6">
                            <p class="text-xs font-extrabold text-orange-800 uppercase tracking-widest mb-3 border-b border-orange-200/50 pb-2">A. Verifikasi Administrasi</p>
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                                @php
                                    $adminChecks = [
                                        'Surat Permohonan' => [$cpcl->is_surat_permohonan_sesuai, $cpcl->ket_surat_permohonan],
                                        'KTP Ketua & Anggota' => [$cpcl->is_ktp_sesuai, $cpcl->ket_ktp],
                                        'SK Kelompok Tani (Kades)' => [$cpcl->is_sk_desa_sesuai, $cpcl->ket_sk_desa],
                                        'Data Simluhtan' => [$cpcl->is_simluhtan_sesuai, $cpcl->ket_simluhtan],
                                        'Notula & Daftar Hadir Rapat' => [$cpcl->is_notula_rapat_sesuai, $cpcl->ket_notula_rapat],
                                        'Titik Koordinat Lokasi' => [$cpcl->is_titik_koordinat_sesuai, $cpcl->ket_titik_koordinat],
                                        'Tidak Menerima Bantuan Sama' => [$cpcl->is_tidak_menerima_bantuan_sama, $cpcl->ket_tidak_menerima_bantuan_sama],
                                    ];
                                @endphp
                                @foreach($adminChecks as $label => $check)
                                    <div class="flex items-start gap-2">
                                        <div class="mt-0.5">
                                            @if($check[0] === true || $check[0] === 1 || $check[0] === '1')
                                        ...
                                        <div>
                                            ...
                                            @if($check[1])
                                                <p class="text-xs text-gray-500 mt-0.5">Ket: {{ $check[1] }}</p>
                                            @endif
                                        </div>
                                    </div>
                                @endforeach
                        ...
                        {{-- Verifikasi Teknis --}}
                        <div>
                            <p class="text-xs font-extrabold text-orange-800 uppercase tracking-widest mb-3 border-b border-orange-200/50 pb-2">B. Verifikasi Teknis Lahan</p>
                            ...
                            @if($cpcl->catatan)
                            <div class="sm:col-span-2">
                                <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Catatan Lapangan</p>
                                <p class="text-gray-700 text-sm bg-white p-3.5 rounded-xl border border-orange-100 mt-1 leading-relaxed">{{ $cpcl->catatan }}</p>
                            </div>
                            @endif
                        </div>
                        </div>
                    ...
                    <div class="mt-6 pt-6 border-t border-gray-100">
                        <p class="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3">Dokumentasi Foto Lapangan</p>
                        <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                            @foreach($proposal->surveyDocumentations as $doc)
                            <a href="{{ Storage::url($doc->file_path) }}" target="_blank" class="block rounded-xl overflow-hidden border border-gray-200 hover:opacity-85 transition-opacity group">
                                <img src="{{ Storage::url($doc->file_path) }}" alt="{{ $doc->keterangan }}" class="w-full h-24 object-cover">
                                @if($doc->keterangan)<p class="text-[10px] text-gray-500 font-semibold px-2.5 py-1.5 bg-white group-hover:text-gray-700 transition-colors">{{ $doc->keterangan }}</p>@endif
                            </a>
                            @endforeach
                        </div>
                    </div>
                ...
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 sm:p-8">
                    <div class="flex items-center justify-between mb-5 border-b border-gray-100 pb-3 gap-2">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <a href="{{ route('kabid.berita-acara.show', $proposal) }}" class="text-xs text-amber-600 font-bold hover:underline">Lihat Detail →</a>
                        </div>
                    </div>
